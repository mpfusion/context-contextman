...
%% \shortsetup{startstandardmakeup}
...
proto¾e mívají jinak velké stránkové zrcadlo ne¾ hlavní textová èást
dokumentu. Pomocí dvojice \type{\start ... \stopstandardmakeup} mù¾eme
navrhnout vzhled strany v~rámci standardního rozmìru papíru. 
...
abychom vysázeli zadní stranu titulního listu.
...
Ná¹e vlastní titulní strana mù¾e být vytvoøena pomocí
...
zpùsobem. Ji¾ jsme vidìli pou¾ití \type{\placefigure} a
\type{\placetable}. Oba jsou pøíkladem plovoucích objektù popsaných
...
Dále mù¾eme nastavit formátování plovoucích objektù pomocí pøíkazu
...
Tyto pøíkazy pou¾íváme v~úvodní èásti vstupního souboru a mají
globální vliv na v¹echny plovoucí objekty.
...
Jinou èástí textu je {\em blok textu}. Pøíkladem bloku textu je jeden
nebo více odstavcù, které chceme pou¾ít nìkolikrát.
...
Mezi hranaté závorky vlo¾íme jeho referenèní jméno. Také mù¾eme
najednou definovat nìkolik blokù textu, pokud mezi jednotlivé jména
napí¹eme èárku.
...
%% available:
Po zadefinování je k~dispozici následující pár pøíkazù:
...
Zde je pøíklad pou¾ití bloku textu:
...
\beginholandstina[blok-h]
Sinds 1810 veroorzaakte de Dedemsvaart enige welvaart in Hasselt.
Alle schepen voeren door de grachten en de winkels aan weerszijden
van de gracht floreerden.
\endholandstina

\useblocks[castina][blok-c]
\stopbuffer
...
vytvoøit dvoujazyèný manuál. Pro tyto úèely je také mo¾no ulo¾it bloky
...
Je mo¾ná také kombinace textu a linek:
...
%% useful.
Je¹tì máme k~dispozici nìkolik doplòkových pøíkazù, které mohou být
velice u¾iteèné:
...
Uveïme je v~následujícím pøíkladu:
...
Tyto pøíkazy bývají pou¾ívány v~dotaznících. Podrhnutý nebo
pøe¹krtnutý text nebude rozdìlen na konci øádky.
...
úhel natoèení: \type{rotation=90}. Slo¾ene závorky vymezují text èi
objekt, který chceme otoèit.
...
\placefigure
  [][fig:rotation]
  {Rotace rybáøského pøístavu (de Vispoort) o~180 \Degrees.}
  \rotate[rotation=180]{\externalfigure[ma-cb-15][width=10cm]}
\stopbuffer
...
Na\in{obrázku}[fig:rotation] mù¾ete vidìt, ¾e není v¾dy úplnì zøejmé,
co dostanete.
...
kdy¾ napí¹eme
\goodbreak\type{\inmargin{uvnitø\\marginálie}}, pak se text zlomí
do dvou øádkù.
...
%% shorthand versions:
Ke zmìnì jednoho jazyka na jiný pou¾ijeme krat¹í variantu:
...
%% your input file:
Pokud je slovo rozdìleno ¹patnì, mù¾eme si definovat místo rozdìlení
...
\index{vstup jiného \TEX--souboru}
...
souboru. Napøíklad je nìkdy efektivnìj¹í rozdìlit kód \CONTEXT{}u do
více souborù, aby mohly být zpracovány samostatnì.

%% Another file (with the name \type{another.tex}) can be
%% inserted by:
Soubor se jménem \type{another.tex} mù¾e být vlo¾en pomocí:
