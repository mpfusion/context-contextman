...
dokumentu. Èasto k~návrhu vzhledu takové výjimeèné strany vyu¾íváme
...
Na¹e vlastní titulní strana mù¾e být navr¾ena pomocí pøíkazù
...
Plovoucím objektem (floating block) se v~\CONTEXT{}u nazývá textový
...
Slo¾ené závorky se pou¾ívají pro jméno objektu v~jednotném
a~mno¾ném èísle. Napøíklad:
...
\startframedtext
Na zaèátku tohoto století existovala tramvajová linka z~Zwolle do
...
zru¹ena. Nyní by taková tramvajová linka mo¾ná byla zase zisková.
\stopframedtext
\stopbuffer
...
jednotlivá jména napí¹eme èárku.
...
Chcete-li vidìt, co vám Hasselt mù¾e nabídnout, mìli byste jej
nìkdy nav¹tívit. Jestli¾e si s~sebou vezmete tento manuál,
nìkterá místa poznáte.
\stopbuffer
...
%% buffer.
Pøíkazem \type{\getbuffer[navsteva]} se vyvolá ulo¾ený text. Referenèní
...
\type{\thinrules} nesmí být opomenuty a~výsledné vertikální odsazení by
mìlo být v¾dy pøedmìtem na¹í pozornosti. 
...
V~tomto textu pøe¹krtnìte \overstrikes{Hasselt}\periods[18]
\stopbuffer
...
Tyto pøíkazy bývají pou¾ívány v~dotaznících. Omezením je, ¾e podtr¾ený
...
Obèas máme potøebu umístit text na konkrétní místo uvnitø strany èi
...
Kulaté závorky urèují souøadnice $x,y$, slo¾ené závorky ohranièují
...
Zvolit mù¾eme jednotky a~mìøítko. Zde je ilustraèní pøíklad:
...
%% \stopbuffer
\startbuffer
V~roce 1252 Hasselt získal mìstská práva. Od té doby mìl
...
svatost a byl zobrazován bez svatozáøe.
\stopbuffer
...
\placefigure
  [][fig:rotation]
  {Rybáøský pøístav (de Vispoort) otoèený o~180 \Degrees.}
  \rotate[rotation=180]{\externalfigure[ma-cb-15][width=10cm]}
\stopbuffer
...
Na \in{obrázku}[fig:rotation] vidíte, ¾e není v¾dy úplnì zøejmé,
...
%% \stopbuffer
\startbuffer
Na radnici mìsta Hasselt je døevìná deska, kde se mù¾eme doèíst:
...
Tato malá básnièka obsahuje varování pro radní Hasseltu: {\it Nedovol,
aby osobní prospìch nebo pocity ovlivòovaly tvùj rozum pøi rozhodování.}
\stopbuffer
...
V~nìkolika pøíkazech se konce øádkù oznaèují \type{\\}. Napøíklad
kdy¾ napí¹eme
...
dìlení slov se jazyk od jazyku li¹í.
...
%% \stopbuffer
\startbuffer
Pokud se chcete dozvìdìt více o~Hasseltu, asi nejlep¹í knihou,
kterou si mù¾ete pøeèí¹t, je {\nl \em Uit de geschiedenis van
...
%% your input file:
Pokud je slovo rozdìleno nesprávnì, mù¾eme si dodefinovat místo rozdìlení
...
Ve¹kerý text mezi \type{\starttext} a~\type{\stoptext} bude
\CONTEXT{}em zpracován. Nìkdy se v¹ak vyskytne fragment textu,
který zpracovávat nechceme, nebo chceme okomentovat pøíkazy
\CONTEXT{}u.
...
Pokud pøed ka¾dým pøíkazem \type{\input} sma¾eme znak \type{%},
...
V~mnoha situacích chceme do svého zdrojového souboru vlo¾it jiný soubor
\TEX. Nìkdy je napøíklad efektivnìj¹í rozdìlit text psaný v~\CONTEXT{}u do
