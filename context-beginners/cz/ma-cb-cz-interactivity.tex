...
Dokumenty dnes mohou být zpøístupòovány elektronicky prostøednictvím
obrazovky poèítaèe.
...
kliknutí na èíslo strany v~rejstøíku a~následné zobrazení odpovídající
stránky dokumentu.
...
Interaktivní chování do jisté míry závisí na programu, kterým
...
\CONTEXT\ je velmi mocný nástroj pro generování elektronických nebo
interaktivních dokumentù \PDF. V~této kapitole je nicménì popsáno
jenom pár standardních vlastností. Proto¾e autoøi tohoto manuálu
chtìjí v¹echny manuály vìnované \CONTEXT u zpøístupnit elektronicky
vèetnì zdrojových textù, je jednou z~mo¾ností, jak se dùvìrnì obeznámit
se schopnostmi \CONTEXT{}u, zpìtné in¾enýrství.\footnote{Pozn. pøekl.:
...
zpùsobí, ¾e se zaènou automaticky generovat hyperlinky a aktivní
slova se zobrazí zelenì.
...
bratranci, proto¾e hyperlinky zabírají nìjaké místo. Také si
v¹imnìme, ¾e zpracování dokumentu probíhá pomaleji. Proto je výhodné
interaktivní mód bìhem pøípravných fází tvorby dokumentu deaktivovat.

%% \section{Interaction within a document}
\section{Interaktivita v~rámci jednoho dokumentu}

%% \index{interaction+internal}
\index{interakce+vnitøní}
...
\type{\at}. Mo¾ná jste se divili, proè pí¹eme
...
V~interaktivním módu máme k~dispozici jeden pøíkaz, který
v~papírové variantì dokumentu nemá pøíli¹ význam. Je jím
...
hranaté odkaz (referenèní jméno nebo pokyn).
...
V~interaktivním dokumentu bude slovo {\em Hasseltu} zelené a
...
%% This is done by:
Je mo¾né odkazovat z~jednoho dokumentu na jiný. Nejdøíve musíme
...
%% \type{PreviousPage}.
Tato definice vytvoøí na pravé stranì ka¾dé strany menu. Tlaèítka menu
budou obsahovat texty {\em Obsah}, {\em Rejstøík} a {\em Konec}
s~odpovídajícími funkcemi, po øadì: skok na obsah, skok na rejstøík
a~ukonèení prohlí¾eèe. Návì¹tí na obvyklé doskoky, jakými jsou
...
Akce typu \type{ExitViewer} (ukonèi prohlí¾eè) je nezbytná, aby ná¹
