...
doporuèejeme vám ke studiu napøíklad: \footnote{V~ tìchto
instrukcích o~ sazbì matematiky se opíráme o~ kní¾ku
...
Kromì tohoto bude v~brzké dobì vydán \CONTEXT ový matematický
modul a k~tomu pøíslu¹ná dokumentace.
...
V~bì¾ných situacích se zvyklosti pro sazbu bì¾ného a matematického
 textu li¹í. \TEX\ tyto zvyklosti zná a bìhem vytváøení dokumentu
je pøesnì aplikuje. Pøi po¾adavcích na vysokou kvalitu sazby matematického
výstupu se mù¾eme na \TEX\ plnì spolehnout.
...
\item Urèité symboly mají rùzný vzhled v~øádkovém a v~zobrazovacím módu.
...
Pøi sazbì matematiky musíte pracovat v~takzvaném matematickém
re¾imu, ve kterém jsou výrazy definovány pomocí pøíkazù
plain \TEX u.
...
Matematický re¾im má dvì alternativy: textový re¾im a zobrazovací
re¾im. Textový matematický re¾im je aktivován pomocí \type{$} a
\type{$}, zatímco zobrazovací matematický re¾im je oznaèen pomocí
\type{$$} a \type{$$}.
...
Pokud budeme uva¾ovat kruhovou oblast této velikosti s~
Hasseltským tr¾i¹tìm v~geometrickém støedu $T$, mù¾eme 
spoèítat prùmìr této oblasti pomocí vzorce ${{1}\over{4}} \pi d^2$.
\stopbuffer
\typebuffer

%This will become:
Bude vypadat po vysázení takto:
...
jednotlivých operací. Pokud zapomenete na vnìj¹í závorky
a zapí¹ete: \type{${1}\over{4} \pi d^2$}, dostanete ne¾ádoucí
...
Písmena a èíslice jsou sázena ve tøech velikostech:
velikost textu $a+b$, velikost script: $\scriptstyle a+b$ a 
velikost scriptscript $\scriptscriptstyle a+b$. Toto lze
ovlivòovat pøíkazy \type{\scriptstyle} a \type{\scriptscriptstyle}.
...
Symboly jako $\int$ a $\sum$ vypadají jinak v~re¾imu textovém
a jinak ve zobrazovacím. Pokud napí¹eme \type {$\sum_{n=1}^{m}$}
nebo \type {$\int_{-\infty}^{+\infty}$}, dostaneme 
{$\sum_{n=1}^{m}$} a {$\int_{-\infty}^{+\infty}$}. To samé
ve zobrazovacím re¾imu: \type {$$\sum_{n=1}^{m}$$} 
...
Pro sazbu zlomkù mù¾ete pou¾ít pøíkaz \type{\over}.
V~\CONTEXT u mù¾ete pou¾ívat také alternativu
\type {\frac}. Napøíklad pro vysázení ${\frac{a}{1+b}}+c$
staèí zapsat: {${\frac{a}{1+b}}+c$}.
...
následující znak, tak¾e pro víceznaké indexy je nutné ohranièovat
závorkama $\{$~$\}$.
...
Ve zobrazovacím re¾imu podléhají automatickému pøizpùsobení
...
Pokud ve zobrazovacím re¾imu chceme pou¾ít slo¾ené zlomky,
pak bychom mìli zapsat bì¾ným zpùsobem pouze jeden
...
v~matematickém re¾imu se nevysází bì¾ným zpùsobem mezera 
...
Ohlednì velikosti mezer v~matematickém výrazu,
se mù¾ete na \TEX\ plnì spolehnout. Nicménì v~nìkterých
situacích se vám mù¾e hodit jejich velikost ovlivòovat.
To se dìlá pomocí:
...
U¾ití pøíkazu \type{\prime} \footnote{pozn.pøekladatele: \quote {prime}
je anglický výraz pro èárku v~indexu} hovoøí sama za sebe. 
Pokud byste napøíklad chtìli vysázet $y_1^\prime+y_2^{\prime\prime}$, 
mìli byste zapsat \type{$y_1^\prime+y_2^{\prime\prime}$}.
...
Jednotlivé matematické formule mù¾ete vysázet oèíslovanì pomocí:
...
dostanete výraz který je {\em zobrazen} ve støedu stránky,
...
povinné a slou¾í pro odkazy a také pro zapnutí a vypnutí 
èíslování.
...
\in{Formula}[middle one] byla vysázena takto:

\startbuffer
\placeformula[prostredni formule]
  \startformula
     y=x^3
  \stopformula
\stopbuffer
...
Nápis v~hranaté závorce \type{[prostredni formule]} slou¾í
pro odvolání se na tuto formuli v~následujícím textu. Tuto 
odvolávku jsme provedli pøíkazem \type{\in{Formula}[prostredni formule]}.

%If no numbering is required you type:
Jestli¾e není vy¾adováno èíslování, zapí¹ete:
...
Èíslování formulí se nastavuje pomocí \type{\setupnumbering}. 
V~tomto manuálu je èíslování nastaveno
pøíkazem \type{\setupnumbering[way=bychapter]}. To znamená,
¾e pøed èíslem formule je uvedeno èíslo kapitoly a v~ka¾dé
dal¹í kapitole je èíslování opìt vynulováno. Z~dùvodu 
jednotnosti jsou tabulky, obrázky apod. èíslovány stejným 
zpùsobem. Proto se \type{\setupnumbering} pou¾ívá pro 
nastavení oblasti va¹eho vstupního souboru.

%Formulae can be set up with:
Formule mohou být nastaveny pomocí pøíkazu:
