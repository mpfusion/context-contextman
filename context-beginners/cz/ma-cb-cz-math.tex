...
tato kapitola týkající se sazby matematiky, není, jak
mo¾ná oèekáváte, rozsáhlá. Pokud potøebujete v~\TEX u sázet vzorce,
doporuèejeme vám ke studiu:\footnote{V~tomto
...
%\index {display mode}
%\index {text mode}
\index {matematický re¾im}
\index {zobrazovací re¾im}
\index {textový re¾im}
...
V~bì¾ných situacích se zvyklosti pro sazbu bì¾ného a matematického
textu li¹í. \TEX\ tyto zvyklosti \quote{zná} a~bìhem vytváøení dokumentu
je pøesnì aplikuje. Pøi po¾adavcích na vysokou kvalitu sazby matematického
výstupu se mù¾eme na \TEX\ spolehnout.
...
\item Urèité symboly mají rùzný vzhled jsou-li uvedeny v~odstavci èi
  na samostatném øádku.
...
a~display matematický mód. Vnitøní matematický mód je aktivován pomocí
\type{$} a~\type{$}, zatímco display oznaèen pomocí \type{$$} a~\type{$$}.
...
a~velikost indexu indexu (scriptscript) $\scriptscriptstyle a+b$. Toto
lze ovlivòovat pøíkazy \type{\scriptstyle} a \type{\scriptscriptstyle}.
...
\type {$$\int_{-\infty}^{+\infty}$$} bude v¹ak vypadat takto:
...
následující znak, tak¾e pro víceznakové indexy je nutné ohranièovat
slo¾enýma závorkama $\{$~$\}$.
...
\type{\Biggl} a jejich pravých ekvivalentù. Pokud to 
...
%will get:
Dále bychom mìli pou¾ívat pøíkaz 
...
musíme vzít na vìdomí dvì vìci. První je skuteènost, ¾e
v~matematickém módu se nevysází bì¾ným zpùsobem mezera 
a je nutné si to vynutit pomocí \type{ \ } (zpìtného lomítka).
Druhou vìcí je nutnost zmìnit font, proto¾e bì¾ný text
by se nemìl vysázet v~$matematicke\ kurzive$, ale aktuálním
fontem.
...
%\TEX:
Matematické funkce jako $\sin$ a $\tan$, které musí být
vysázeny aktuálním fontem, jsou v~\TEX u pøeddefinované
funkce:
...
%defined:
Nìkdy je potøeba zarovnávat dle více rùzných míst.
...
U¾ití pøíkazu \type{\prime} \footnote{Pozn. pøekl.: \quote{prime}
je anglický výraz pro èárku v~indexu.} hovoøí sama za sebe. 
...
Na konci této sekce si uka¾eme pou¾ití pøíkazu
...
Jednotlivé matematické formule mù¾eme èíslovat pomocí:
...
%previous examples.
dostaneme výraz, který je {\em zobrazen} ve støedu stránky,
ale není zarovnán jako v~pøedcházejícím pøíkladì.
...
Pøíkaz \type{\placeformula} zajistí mezeru okolo matematické 
formule a~také její oznaèení èíslem. Hranaté závorky nejsou 
