...
In older versions of \CONTEXT, the model for defining fonts that is
...
wrapped inside of those. Most commercial fonts have only one design size,
and when you create a typescript for such fonts, you can simply reuse the
predefined size definitions. Later on we will see that this means you can
just refer to a \type{default} definition.
...
The core of this intermediate model is the definition command that is used
as follows:
...
definitions are needed. Add to that the fact that sometimes fonts with
mixed encodings have to be used, i.e.\ with the glyphs positioned in
different font slots, and you can understand why font handling in \TEX\ is
often qualified as \quote {the font mess}. Flexibility simply has its
price.

Many documents have a rather simple design and use only a couple of (often
related) fonts. For some commonly used fonts, this means that one can stick
to loading the appropriate predefined font definition file. \footnote {The
original font definition files are replaced by typescripts in \type
{type-pre}, but font files are still supported for upward compatibility
reasons.} But font life is seldom simple and, in a worst case scenario, one
must define the fonts in the document style.

Like most other \TEX\ users, Hans Hagen started out using the Computer
Modern Roman fonts. Since these fonts have specific design sizes \CONTEXT\
supports accurate definitions with specific font names and sizes for
each combination. The following is an example of that:
...
Because there are no more sizes in the definition any more, we can just as
well combine all of the requested sizes in a single \type{\definebodyfont}
by using a list of sizes as the first argument. This means exactly the same
as repeating that whole list five (or more) times, but saves a lot of typing:
...
and the default serif fonts are in fact defined with:
...
environments. The first four are required in all math setups just to do
basic formula typesetting, the other four (\type{ma} ... \type{md}) can be
left undefined and are normally used for fonts with special symbols or
alphabets like the AMS symbol fonts \type{msam} and \type{msbm}.
...
There is a small list of typescript files that is tried always,
and you can add extra ones at the end of the list by using
\type{\usetypescriptfile}.
...
Extra arguments to \type{\starttypescript} are not ignored. However, you can force any
key at all to match by using the special keyword \type{all} in your
\type{\usetypescript}.  We will see later that this is sometimes handy.

We have to step back for a moment to the macro \type{\definetypeface}, and
especially to the third, fourth and fifth argument:
...
the predefined combinations? It was said there that these \quote{are
...
actually used as parts of  \type{\usetypescript} calls. Inside the macro
definition of \type{\definetypeface}, there are the following lines:
...
The \type{\typefaceencoding} is inherited from the calling
\type{\usetypescript}, so that the \type{\definetypeface} in our example
expands into:
...
not all of these typescript blocks are indeed present in the list of files that
have to be scanned, but a few are, and some apparently even more than once:
...
This one maps the standard symbolic names to names in the \quote{Palatino} family.
...
This maps the Palatino names onto actual font files. Some further processing is taking
place here: the calling \type{\usetypescript} that was called from within the
\type{\definetypeface} knows that it wants \type{ec} encoding. Because this is the
third argument, it becomes the replacement of \type{\typescriptthree}. The body of the
typescript therefore reduces to:
...
the harddisk. This way, there is a smaller chance of errors creeping in.

The third match is a block that sets sets up \quote{TeXPalladioL} font synonyms. These
will not actually be used, but it is a match so it will be executed.
...
as explained earlier, this runs a named \type{\definebodyfont} that is defined in
\type{type-unk.tex}:
...
Although any keyword is permitted in any of the three arguments, the current
definitions have some reserved (advised) keys:
...
When you take a close look at the actual files in the distribution you will notice a
couple of more keywords.  One in particular is worth mentioning: instead of the
predefined sizes in \type {default}, you can use the \type {dtp} size scripts with their
associated body font environments by 

\starttyping
\usetypescript [size] [dtp] [all]
\stoptyping
