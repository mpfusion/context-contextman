...
found together within a single book design (like for a software manual).

\startbuffer[styles]
\vskip1ex
\starttabulate[|l|l|l|l|l|l|]
\NC \bf \rlap{identifier(s)} \NC \NC   \NC \qquad \NC \bf example        \NC \NR
\NC serif \NC regular  \NC roman       \NC \NC \rmtf main text          \NC \NR
\NC sans  \NC support  \NC sans        \NC \NC \sstf section headings   \NC \NR
\NC mono  \NC teletype \NC type        \NC \NC \tttf code examples      \NC \NR
\stoptabulate
\vskip1ex
\stopbuffer
...
The leftmost two series of names are normally used by typographers and
book designers, the third series is what was traditionally used in
plain \TEX. In \CONTEXT\ all three series of terms can be used
because they are remapped to the same set of internal commands. As
will be explained later, the command \type {\rm} is used to switch to
a roman|/|serif|/|regular style, and \type {\tt} for switching to
mono spaced or typewriter style, etcetera.
...
little under 2.85 points per millimeter, if you prefer ISO
...
bold and italic altermatives when you change the family or
...
of an \type {\tx}. When you switch to italic (\type {\it}),
...
To call these sepcial typescripts, it's most convenient to define a
...
Earlier we have seen that within a single body font there are in fact
different font sizes for example for super|-| and subscripts. The
...
by \CONTEXT\ itself. Allthough the macro syntax says the type is
...
but longer stretches of text, you will probably want to set up most of the
...
properly. Most of the underlying features of \PDFTEX\ can not {\em
just\/} be turned on or off, it is possible to tweak the machinery on
...
is the virtual name for the combination {\rm\tf}. If you also want the
...
font synonyms for virtualized symbolic font names like \type{Serif}
and \type{SerifBold} en block. They will even work with your own
