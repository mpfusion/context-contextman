...
intended to be used together.  Such a set of fonts with  the same basic design is known as a \definition{font family}. For example, Computer Modern is a font family, as is Lucida.

Within a font family there can be multiple \definition{styles}.
In the example below you see five font styles of the Latin Modern family: the Roman,
Sans, Typewriter, Smallcaps and Variable Typewriter. (However, this manual will often continue to
use the word {\em style} in a more generic sense. Hopefully the context will make clear which
sense is intended.)
...
Within a given style, there can be multiple \definition{alternatives} of a font. The example below shows a normal, a bold, an italic, and a bold italic alternative.
...
classified as to their style: \quote{serif} (glyphs strokes have embellishments at the
...
overall style (not necessarily in {\em style}). In \CONTEXT\ you will normally use a reimplementation of
...
\vskip1ex
\starttabulate[|l|l|l|]
\NC \bf \rlap{terms}  \NC \qquad \NC \bf intended usage        \NC \NR
...
\item It should be possible to combine all of these requirements
      ...
      % Need a reference here to a definition of `body font'. It's not clear.
...
thoroughly comprehend the above paragraphs.
% It's not possible to thoroughly comprehend yet! Especially the term `body font'.
\CONTEXT's terminology
...
recap: we call a set of fonts from the same source, with the same basic design, like Lucida or Computer Modern
Roman, a {\em family}.  Within such a family, the members can be
grouped according to characteristics such as the presence of serifs, or the variability of width. Such a group is called a {\em
...
be other  % Careful of the term ''alternative''.
classifications.
% It used to say ''but they all refer to the presence of serifs and the glyphs having equal widths'',
% but that's not true... e.g. handwriting, calligraphic, math.
Within a style there
...
Often a document will be mostly typeset 
using just one combination of family and style. 
This is called the \definition{bodyfont}.
Consistent use of alternative commands like \type {\bf} and \type
{\it} in the text will automatically result in the desired
bold and italic alternatives when you change the family or
style in the setup area of your input file, since these commands
adapt to the specified family and style. 
...
\vskip 1ex
\starttabulate[|l|l|]
% The following row was commented out before, but it's needed.
% Otherwise there's no way to know what the stuff in the right column really means.
\NC \bf command \NC \bf keyword equivalents   \NC\NR
...
\placetable
  [here]
  [tab:style switches]
  {Font style switching commands and their keyword equivalents. For more on keywords, see ~\in{subsection}[subsec:styleKeywords].}
...
Note that, while these alternatives can sometimes seem to be \quote{combined,} as in \type{bolditalic},
it is important to recognize that only one alternative can actually be active at a time. In this regard
alternatives are like \quote{radio buttions.} \type{bolditalic} is
in fact one predefined alternative, not a combination of two. Alternatives cannot be arbitrarily combined,
or turned on and off independently of each other.

Besides these two-letter commands, there is a series of font selector
commands with a size suffix attached. Some examples of that are:
...
The \type{a} suffix selects a somewhat larger font size than the default.
% Is there a more correct way to describe this than ``the default''? Is it the body font size?
Each of the ordered alphabetic suffixes \type{a}, \type{b}, \dots\
select a somewhat larger actual font than the previous suffix. The
...
% Is there a command for selecting the default font size?
% E.g. within a macro where you don't know what's come before.
...
The \quote{small} (single \type{x} suffix) switches mentioned 
in~\in{table}[tab:small alternative switches], such as \type {\tfx}, are always available. 
...
\type {\bf} equals \type {\bfb}, etc. This behavior
% It used to say ``This method'', but that doesn't seem to make sense.
 is almost always preferable over returning to the original character size, but it 
...
\type {\xii}
% Information about these switches is hard to find. What is this referring to?
% Maybe de Wijn's fnts.tex (http://www.staff.science.uu.nl/~wijn0104/fnts.html)?
% However, de Wijn's fnts.tex defines \vii@ and \viipt, but not \vii.
% The latest version of plain.tex (http://www-cs-faculty.stanford.edu/~uno/plain.tex)
% doesn't define these switches.
are also defined; these have local effects like \type{\tfx} and \type{\tfa}. 
% In what sense are the effects local? From what I can tell,
% the effect of \tfx lasts permanently, unless you make it local
% by using a group or something.

A \quote{local} font change means that \type{\tfx} in running text will only change the
font in the running text, not in headers etc.; and will only change the font from
that point, to the end of the enclosing group. In other words, it does not change the
font for the whole document.



\subsection[subsec:styleKeywords]{Switching font styles in setup commands}
...
When you make a font switch without changing the bodyfont, e.g using \type {\tfa},
 the linespacing is not adapted unless you give
the command \hbox{\type {\setupinterlinespace}} without any setup
parameters, or with the key \type {reset}. For example
...
the paragraph is finished (in which case, the paragraph is
ended by the empty line after the end of the group). 

In the following example, the word \type{height} is typeset inside a bare \type{\tfd} 
group without \type{\setupinterlinespace}, to illustrate why \type{\setupinterlinespace} is required:
...
%Here's how it would look {\em with} \type {\setupinterlinespace}:
%
%{
%\startnarrower
%\tfa \setupinterlinespace In books meant for children
...
%typographic knowledge of the user of such a system. The
%interline space sometimes differs on a line by line basis,
%and depends on the {\tfd\setupinterlinespace height} of the current line.
%Therefore, when changing the style, something that should
%only be done on purpose, also change the baseline distance
%when needed. \par 
%\stopnarrower 
%}
% It looks the same to me. What is supposed to be the difference?
...
In a running text a 
% Used to say: ``a *temporary* font switch''
% I don't think this is correct. \switchtobodyfont is only temporary if you
% make it temporary by using a group around it, or by following it with another
% command that changes things back. Right?
font switch is done with the command:

\showsetup{switchtobodyfont}

This command changes the bodyfont in the running text where it occurs,
% when it occurs in running text?
but not in headers and footers. % unless the command occurs in headers and footers?
With \type {small} and \type {big} you switch to a
...
\type{\switchtobodyfont} differs from the use of font selector commands like \type{\ss}, \type{\it} and \type{\tfa}
in a few significant ways. For one, \type{\switchtobodyfont} is ``heavier'': it results in more thorough
changes, for example, adjusting the interlinespace to suit the change
(if the change lasts until the end of a paragraph).
As such, switching the body font is slower. In general, \type{\switchtobodyfont} is intended for longer-lasting
changes, e.g. at least a paragraph, whereas  \type{\it} and the like are meant for changes of short duration,
such as a phrase. Moreover, it's useful to know that \type{\switchtobodyfont}
controls and changes the sizes for commands like \type{\tfa}.
% I think we need more information on that last point.
...
The first argument is optional, and specifies the typeface identifier 
...
\type{DIMENSION},  dimensionless floating point numbers are also acceptable. Such numbers are
...
\placefigure
  ...
  {Output of \tex{showaccents} for the current (palatino) font in \PDFTEX}
