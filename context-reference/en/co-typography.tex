...
\definefontfeature[default][default][protrusion=pure,expansion=quality,mode=node,script=latn]
...
\NC compose   \NC Use fallback composition in \MKIV\ 
                  (see~\in{section}[sec:font fallbacks]) \NC \NR
...
achieved by Knuth's original \TEX.  Although the extensiosn are made
...
\starttabulate[|l|l|]
\NC hanging    \NC turns on character protrusion (see the next section)\NC \NR
\NC nohanging  \NC turns off character protrusion\NC \NR
\NC hz         \NC turns on font expansion (see~\in{section}[sec:hz])\NC \NR
\NC nohz       \NC turns off font expansion \NC \NR
\NC spacing    \NC turns on special spacing rules (see~\in{section}[sec:otherhandling])\NC \NR
\NC nospacing  \NC turns off special spacing\NC \NR
\stoptabulate

Now, let's move on to how to set up the system for font handling
properly. The underlying features of \PDFTEX\ can not {\em just\/} be
turned on or off, it is possible to tweak the machinery on the font
as well as on the individual glyph level. You can define those
settings all on your own, but \CONTEXT\ comes with a handy set of
predefined values.
...
In this first example the text was typeset in the
...
In this example, the protruding machinery is put to work.
...
Also because of this tight connection with the font itself, the
method of setting up the support at the font level is a little
different between \PDFTEX\ and \MKIV.

IN \PDFTEX, setting up the \type{handling} takes place in a similar 
way as encoding, like so:
...
But you can also specify a handler at a higher level of abstraction:
...
In \MKIV, font handling is merged with font the features (because
these already have a low-level connection to the font), so you can
set up the font-side of things with the sixth argument 
of \type{\definetypeface}, or by redefining the feature set 
that is used by the typescript you are using and then (re-)executing 
the typescript, like so:
...
So far we have used so-called \type{pure} protrusion, this extends
the punctuation and hyphenation characters into the margin completely.
The settings that \THANH\ mentions in his thesis for protrusion are
available under the names \type {punctuation} and \type {alpha}:

\starttabulate[|lT|p|]
\HL
\NC \rm\bf vector \NC \bf protruding character set \NC \NR
\HL
\NC punctuation \NC hyphenation and punctuation characters, but using
                    a more subtle approach than \type{pure} 
                    (characters stick out by specific percentages)\NC \NR
\NC alpha       \NC the latin alphabet characters          \NC \NR
\NC extended    \NC the other characters                   \NC \NR
\HL
\stoptabulate

The \type {extended} category maps the special (latin) characters onto the
...
Font protruding is turned off automatically in some particular
situations, more control over that will be added eventually. 
...
inspired on his work. Although official qualified in \PDFTEX\ as
...
pararaphs (protruding and hz), \CONTEXT\ also provides the \type
...
handlings are somewhat experimental.


\subsection {Defining font handlings}

\macro{\tex{startfonthandling}}
\macro{\tex{definefonthandling}}
\macro{\tex{usefonthandling}}
\macro{\tex{defineprotrudefactor}}
\macro{\tex{inheritprotrudefactor}}

At the lowest level, the font handling definitions look like this 
(here showing the default \type{punctuation} handling):
...
The two fractional arguments are a proportion of the width of the
glyph, one for the left shift and one for the right shift. There are
a few more vectors defined, like \type {defalph} for characters,
where only very small shifts are applied.

A combination of such vectors is then packaged to become a font 
...
and such definitions are then loaded with the \type {\usehandling}
command. You could for instance load the default definitions with:

\starttyping
\usehandling[def]
\stoptyping

But since this handling vector is already preloaded in \CONTEXT\
(just like all the others that were mentioned so far), you
normally don't have to provide this command yourself.

As already mentioned, the amount of protruding depends on the shape,
which is why different values may be needed for slanted or bold
shapes. One way of using the same vector for different shapes, is
changing the strength of the protruding:
...
more (probably incompatible) fine tuning in the future. 
