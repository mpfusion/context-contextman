...
The alternatives within a style are given below. The abbreviation
\type {\sl} means {\sl slanted}, \type {\it} means {\it italic} and
\type {\bf} means {\bf boldface}.   Sometimes \type {\bs} and \type
{\bi} are also available, meaning {\bs bold slanted} and {\bi bold
italic}. 

With \type {\os} we tell \CONTEXT\ that we prefer mediaeval or
old||style numbers {\os 139} over {\rm 139}. The \type {\sc}
generates {\sc Small Caps}. The base font style (upright) 
is selected by the command \type {\tf}.
...
\type{x} and \type{xx} suffixes select a smaller version.

It depends on the completeness of the font definition files whether
all alternatives like \type {\ita}, \type {\bfx}, \type {\bfa}, etc.
are available. For the core \CONTEXT\ fonts, you can count on at least
\type{\tfa}, \type{\tfb}, \type{\tfc}, \type{\tfd}, \type{\tfx}, 
and \type{\tfxx} being defined. For the others, just try and see what
happens.

A more generic downscaling command is also available:
...
This is the best moment to explain a little trick: because the various \type{type-xxx}
...
will result in \PDFTEX\ using the Type~1 font Pagella from the \TeX\ Gyr projecte instead 
...
In the next section there is a table (\in {}[tab:typeface names]) that explains for each typescript
what font set it matches each of the styles to.
...
\placetable
  ...
  {The typescripts.\crlf 
   Unless stated otherwise, style {\bf rm} uses serif, style {\bf ss} uses sans, style {\bf tt} uses
