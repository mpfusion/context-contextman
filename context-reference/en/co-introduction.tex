...
\TEX\ was developed at the Stanford University during the
...
The development of \CONTEXT\ was started in 1990. A number
of \TEX\ based macropackages had been used to our
...
production of a great number of complex educational
materials and workplace manuals and handbooks. In 1994 the
package was stable enough to warrant a Dutch user manual.
Over the years \CONTEXT\ has been upgraded with many
features and German and English interfaces were added.
Though \CONTEXT\ is as (un|)|stable as any other
macropackage there are still a great number of wishes. These
...
A \CONTEXT\ document is normally coded in \ASCII. Such a
document consist of text mixed with \CONTEXT\ commands.
These commands tell the system how the text should be
typeset. An example of such a command is \type {\sl}. A
\CONTEXT\ command begins with a backslash (\type {\}). Most
of the time a command does something with the text that
comes after the command. The text after the command \type
{\sl} will be typed {\sl slanted}.

When we use a command like \type{\sl} we are typesetting.
Typesetting and writing are conflicting activities. As an
author you would rather spend as little time as possible
with typesetting. However, you want to indicate that
something has to happen with the text. An example is a
command like \type{\em} ({\em emphasis}). Commands like
\type{\em} enable the typesetter to change the meaning of
this command without having to edit the text.

\placefigure[left][fig:cohesion tex]{}{\getbuffer[fig-cont]}

A \TEX\ user normally speaks of macros instead of commands.
A macro is a, often small, program. Although we will use
both \quote {command} and \quote {macro}, we try to
consistently use the word command for users and macro for
programmers.

A collection of macros is called a macropackage. We believe
\CONTEXT\ is one of the most extensive and complete
...
are placed between brackets (\type{[]}). The scope or range
of the command, the text acted upon, is placed between curly
brackets (\type{{}}). For example:
...
In both examples the setups are placed between \type{[]}.
Several setups are defined in a comma list. A number of
examples are:
...
Often keywords or key||value pairs can be passed, that inform
\CONTEXT\ on the users wishes:
...
text is allowed. In our opnion, this increases readability
...
\type{[text]}. Optional arguments are displayed as {\sl
slanted} text. Default values are \underbar{underlined} and
possible alternatives are typeset {\it slanted}. In this
example {\sl text} means that you can provide any
footertext. \CONTEXT\ is able to keep track of the status of
information on the page, for instance the name of the
current chapter. We call this kind of information {\sl
mark}, so the command \type {\setupfootertexts} accept
references to marks, like those belonging to sectioning
commands: \type{chapter}, \type{section}, etc. The argument
\type{date} results in the current systemdate.
