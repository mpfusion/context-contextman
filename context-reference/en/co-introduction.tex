...
{\italic} will be typeset {\italic text in italic}.

When you use a command like \type{\italic} you are acting as a typesetter,
...
brackets (\type{{}}); there may be more than one of those as well.  (Note that the word \quote{argument} in this manual is sometimes used both for setups and for the text acted upon.)
...
where \type{something} indicates a predefined keyword such as \type{narrower}. Often, keywords or key||value pairs can be passed, that inform
\CONTEXT\ of the user's wishes, like
...
is sufficient to convert your \ConTeXt\ source in \type{myfile.tex} to the
\PDF\ file \type{myfile.pdf}. Several command line options control the
\ConTeXt\ run. For a complete list of options, you can run
...
\starttabulate[|lT|lp|]
\HL
\NC \bf option \NC \bf meaning \NC \NR
\HL
\NC --result=name \NC rename the resulting output to the given name \NC \NR
\NC --mode=list   \NC enable given the modes (conditional processing
in styles \NC \NR
\NC --once \NC only run once (no multipass data file is produced) \NC \NR
\NC --nonstopmode   \NC run without stopping \NC \NR
\NC --version   \NC report installed context version \NC \NR
\HL
\stoptabulate

\todo{Maybe some more explainations about \type{texexec} and \type{context}
  here, maybe from a text editor or environment like texworks.}
...
defines a \type{something} command called \type{\name}. For example,
...
defines a \type{head} command called \type{\section}. The name can be arbitrarily chosen by the user; but the type of command must be a predefined one.
...
This defines a \type{head} command called \type{\section}, which inherits characteristics from the existing \type{head} command called \type{\chapter}. It then defines setups for the new \type{\section} command.
...
second command we define a picture named \type{Logo}, to be called later, that is scaled to a \type
...
The last example has optional arguments between optional 
(\type{[]}). Many commands have optional arguments. In case
these optional arguments are left out the default values
become operative.

You may have noticed that you are allowed to have plenty of whitespace in your \ASCII\ text (within certain limits). In our opinion, this increases readability
...
in detail in a later chapter, has three (or more) setup arguments, of which
the first is optional. Optional arguments are displayed with the word
 {\ttxx OPTIONAL} below the brackets. Multiple alternative values are listed for each argument; the first argument can either \type{[text]}, \type{[margin]}, or \type{[edge]}. It defaults to \type{[text]}. Default values are
\underbar{underlined} and placeholders (as opposed to literal
keywords) are typeset in {\ttx UPPERCASE}.
In this example, {\ttx TEXT}
...
%\index{extensions}
\index[utf]{\UTF}
\todo{Where is a better place to write about \TABLE ? The old paragraph:
...
\CONTEXT\ is used with \UTF-8 source files. \UTF-8 is a character encoding,
that can represent every character in the Unicode character set, and that is
backward-compatible with \ASCII. The \UTF-8 file with the prescribed extension
\type{tex} is processed by \CONTEXT. During this process \CONTEXT\ produces a
\PDF\ (Portable Document Format) output file. \PDF\ files are of high
graphical quality and are also interactive (hyperlinked).
...
It is highly recommended, that all input files, i.e.\ the \ConTeXt\ source and
other included files such as image files, have only the letters a--z, digits
and dashes in their names, that is in the names of their full paths, otherwise
you can easily get into problems. Especially the characters *, ?, the space
character and some more are known to cause problems, sometimes because of
\ConTeXt\ itself (internal pattern matching), sometimes because of issues with
the underlying operating system. The dot \quote{.} is somewhat special: it's
used to separate the suffix from the rest of the file-name, but at other
places in the path it can cause trouble just like \quote{*} or \quote{?}.
...
Each of these characters can be typeset by
...
indicates the beginning of the mathemathical mode), a \# is displayed by
\type{\#} (in \TEX, \type{#} stands for arguments in macro definitions),
and a pair of \{ and \} without leading backslashes define the opening and
closing of a group. Finally the backslash~\backslash\ itself is entered by
typing \type{\backslash}.

\todo{What about other \quote{reserved} symbols?
